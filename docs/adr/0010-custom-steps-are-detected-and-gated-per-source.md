...
`install --dry-run` may execute `check` to build the plan, but never executes Custom `install` or `uninstall` operations and never writes artifacts.
...
If an installed Custom step does not declare `uninstall`, the operation leaves its effects intact, reports `uninstall-unsupported` as a warning, and preserves its Installation record entry. The absence of `uninstall` is valid and does not by itself make the command fail.

An external Custom `script` path is resolved relative to the Recipe directory. It may use parent segments to reach shared files elsewhere within the Source, but normalization must prove that the resolved path remains inside the Source root; absolute paths and escapes beyond the Source are rejected. Inline `content` has no source-file path to validate.
...
Custom operations have finite timeouts. The defaults are 60 seconds for `check` and 30 minutes for `install` or `uninstall`; each runtime candidate may override its operation timeout. Expiration terminates the process and produces an error, subject to the Step's optionality rule.
