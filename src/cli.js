...
import { isRepositoryUrl } from "./git.js";
...
function parseCatalogCommand(args) {
    ...
    if (subcommand !== "add" &&
        ...
        return {
            ok: false,
            message: "Expected the catalog add, list, info, search, or remove command",
        };
    }
    try {
        ...
        const expected = subcommand === "list"
            ? [0]
            : subcommand === "info" || subcommand === "remove"
                ? [1]
                : [1, 2];
        if (!expected.includes(positionals.length) ||
...
function parseCommandLine(argv, cwd) {
    if (argv[0] === "catalog")
        return parseCatalogCommand(argv.slice(1));
    if (argv[0] !== "doctor" &&
        argv[0] !== "install" &&
        argv[0] !== "uninstall") {
        return {
    ...
    const dashedRoot = args.findIndex((arg, index) => arg === "--root" &&
    ...
    try {
        ...
        if (tokens.filter((token) => token.kind === "option" && token.name === "root")
            .length > 1) {
            return { ok: false, message: "--root may only be specified once" };
        }
        if ((command === "install" || command === "uninstall") &&
            tokens.filter((token) => token.kind === "option" && token.name === "force").length > 1) {
            return { ok: false, message: "--force may only be specified once" };
        }
        if (command === "install" &&
            tokens.filter((token) => token.kind === "option" && token.name === "update-lock").length > 1) {
            return { ok: false, message: "--update-lock may only be specified once" };
        }
        if (command === "install" &&
            tokens.filter((token) => token.kind === "option" && token.name === "frozen-lockfile").length > 1) {
            return {
                ok: false,
        ...
        const allowCustom = (values["allow-custom"] ?? []).map((value) => (isRepositoryUrl(value) ? value : resolve(cwd, value)));
        if (command === "doctor") {
        ...
        return {
            ...
            options: {
                ...
                force: values.force ?? false,
                updateLock: values["update-lock"] ?? false,
                frozenLockfile: values["frozen-lockfile"] ?? false,
                allowCustom,
            },
        };
    }
...
function renderCatalogHuman(envelope) {
    const lines = [`status: ${envelope.status}`];
    for (const action of envelope.actions) {
        lines.push(`${action.state}: ${action.name ?? ""}${action.path === undefined ? "" : ` ${action.path}`}`.trim());
    }
    for (const catalog of envelope.catalogs ?? []) {
        lines.push(`catalog: ${catalog.name} ${catalog.path} (${catalog.valid ? `${catalog.entries ?? 0} entries` : "invalid"})`);
    }
    if (envelope.catalog !== undefined) {
        lines.push(`catalog: ${envelope.catalog.name ?? "unregistered"} ${envelope.catalog.path}`);
        for (const entry of envelope.catalog.entries) {
            lines.push(`entry: ${entry.title} — ${entry.description}`);
        }
    }
    for (const result of envelope.results ?? []) {
        lines.push(`match: ${result.catalog} ${result.title} — ${result.description}`);
    }
...
function isCatalogEnvelope(envelope) {
...
function renderHuman(envelope) {
    ...
    const lines = [`status: ${envelope.status}`];
    for (const action of envelope.actions) {
        lines.push(`${action.state}: ${action.type}${action.target === undefined ? "" : ` ${action.target}`} (${action.source}/${action.recipe} step ${action.step})`);
    }
    for (const diagnostic of envelope.diagnostics) {
        const context = [
            diagnostic.document,
...
export async function main(argv = process.argv.slice(2)) {
    ...
    process.stdout.write(command.options.json
